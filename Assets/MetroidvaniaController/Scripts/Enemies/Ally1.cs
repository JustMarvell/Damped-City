...
public class Ally1 : MonoBehaviour
{
	...
	public float melleAttackRadius = 0.9f;
	public float melleAttackCooldown = .5f;
	...
	public bool showMelleDist = false;
	...
	void FixedUpdate()
	{
		...
		else if (enemy != null)
		{
			...
			else if (!isHitted)
			{
				...
				if (Physics2D.OverlapCircle(transform.position, detectionRadius, targetLayer) != null)
				{
					if (Mathf.Abs(distToPlayer) < 0.25f)        // player ada di dekat
					{
						// kse bdiam dp enemy
					...
					else
					{
						if (!endDecision)
						{
							...
							if (randomDecision < 0.4f)
								...
								StartCoroutine(Dash());
							else if (randomDecision >= 0.8f && randomDecision < 0.95f && canThrow)
								RangeAttack();
							else
								Idle();
						}
	...
	public void MeleeAttack()
	{
		...
		Collider2D[] collidersEnemies = Physics2D.OverlapCircleAll(attackCheck.position, melleAttackRadius);
		for (int i = 0; i < collidersEnemies.Length; i++)
		{
			...
			{
				collidersEnemies[i].gameObject.GetComponent<CharacterController2D>().ApplyDamageADV(2f, transform.position, knockbackForce, stunTime);
			}
		}
		StartCoroutine(WaitToAttack(melleAttackCooldown));
	}
	...
	void OnDrawGizmosSelected()
	{
		...
		if (attackCheck != null && showAttackRadius)
		{
			...
			Gizmos.DrawWireSphere(attackCheck.position, melleAttackRadius);
		}
